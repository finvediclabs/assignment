...
if (isset($_POST["exid"])) {
    ...
    for($i=1; $i<=$nq; $i++){
        $qid_raw = isset($_POST['qid' . $i]) ? $_POST['qid' . $i] : '';
        $qid = mysqli_real_escape_string($conn, $qid_raw);
        $op_raw = isset($_POST['o' . $i]) ? $_POST['o' . $i] : '';
        $op = mysqli_real_escape_string($conn, $op_raw);
